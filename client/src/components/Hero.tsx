...
const heroVideos = [
  { title: "AIR", url: "/images/work/ALR_reel_v01.mp4" },
  { title: "AWS DHRUwo", url: "/images/work/AWS%20Fargate%20V1.mp4" },
  { title: "DHRUwo", url: "/images/work/Dhurwo%20Cabs%20Test_1.mp4" },
  {
    title: "dhurvo",
    url: "/images/work/dhruwo%20final%20digiteller%20version.mp4",
  },
  { title: "oldays", url: "/images/work/oldays%20reel%201.mp4" },
  { title: "paperboat", url: "/images/work/PaperBoat_story.mp4" },
  { title: "reelestate", url: "/images/work/Real_estate.mp4" },
  { title: "seduction", url: "/images/work/Seduction%20body%20lotion.mp4" },
  { title: "smash", url: "/images/work/SMASH_19.08_v1.mp4" },
  {
    title: "T&s",
    url: "/images/work/T%26S%20Behala_18.07.25_Reel_%20final.mp4",
  },
  { title: "T&s", url: "/images/work/TS.VS8.19.07.mp4" },
  { title: "zomatoanim", url: "/images/work/zomatoanim.mp4" },
];

// 15 portfolio items cycling through the requested videos
const portfolioItems: PortfolioItem[] = Array.from({ length: 15 }, (_, i) => {
  const item = heroVideos[i % heroVideos.length];
  const posterUrl = item.url.replace(/\.(mp4|webm|ogg)(\?.*)?$/i, "-poster.jpg");
  return {
    id: String(i + 1),
    imageUrl: item.url,
    posterUrl,
...
const WordsReveal = memo(function WordsReveal({
  text,
  className = "",
  floating = false,
}: {
    ...
    >
      {words.map((word, i) => (
        <motion.span
          ...
          animate={floating ? { y: [0, -2, 0] } : undefined}
          transition={
            floating
              ? { duration: 8, repeat: Infinity, ease: "easeInOut", delay: i * 0.06 }
              : undefined
          }
        >
          {word}
...
export function Hero() {
  ...
  useLayoutEffect(() => {
    ...
    (async () => {
      ...
      if (!leftTextEl) return;

      gsap.registerPlugin(ScrollTrigger);

      const sectionEl = sectionRef.current ?? leftTextEl;

      const titleEl = leftTextEl.querySelector<HTMLElement>(
        ".hero-left-title",
      );
      const titleWrapEl =
        leftTextEl.querySelector<HTMLElement>(
          ".hero-left-title-wrap",
        ) ?? titleEl;
      const subtitleEl = leftTextEl.querySelector<HTMLElement>(
        ".hero-left-subtitle",
      );
      const descEl = leftTextEl.querySelector<HTMLElement>(
        ".hero-left-desc",
      );
      ...
      let floatWords: any;
      let breatheTracking: any;
      let floatSprinkles: any[] = [];
      ...
      const ctx = gsap.context(() => {
        ...
        floatWords = gsap.to(split.words, {
          yPercent: "random(-6, 4)",
        ...
        breatheTracking = gsap.to(titleEl, {
        ...
        floatSprinkles = sprinkleEls.map((el, i) => {
          const d1 = gsap.to(el, {
            x: `random(-14, 14)`,
            y: `random(-16, 16)`,
        ...
        if (sectionEl) {
          activeTrigger = ScrollTrigger.create({
            ...
            onLeave: () => {
              floatWords?.pause();
              breatheTracking?.pause();
              floatSprinkles.flat().forEach((t: any) => t.pause());
            },
            onLeaveBack: () => {
              floatWords?.pause();
              breatheTracking?.pause();
            ...
            onEnter: () => {
              floatWords?.play();
              breatheTracking?.play();
              floatSprinkles.flat().forEach((t: any) => t.play());
            },
            onEnterBack: () => {
              floatWords?.play();
              breatheTracking?.play();
  ...
  "
            ...
            >
              ...
              <div className="hero-left-title-wrap relative inline-block">
                ...
                <motion.h1
                  ...
                  className="hero-left-title font-azeret font-bold uppercase tracking-tight leading-[0.98] drop-shadow-[0_18px_40px_rgba(13,18,60,0.45)]"
              ...
              <p className="hero-left-subtitle text-lg sm:text-xl md:text-2xl lg:text-2xl font-semibold tracking-tight text-brand-beige-100/90 max-w-md mx-auto lg:mx-0 lg:max-w-xl leading-relaxed">
                <WordsReveal text="Tailor of Tales. One stop Creative Solution." floating={false} />
              </p>
      ...
      "
              >
                <WordsReveal
                  text="your extended creative partner. Its like getting premium for design and media."
                  floating={false}
                />
              </p>
